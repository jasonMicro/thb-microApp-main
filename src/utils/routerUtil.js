...
function loadRoutes(routesConfig) {
  ...
  if (routesConfig) {
    ...
    // 更新应用到路由
    store.getters['account/routesConfig'].forEach((item) => {
    ...
    routesConfig = menuConfig(store.getters['account/routesConfig']);
  }
  // 如果开启了异步路由，则加载异步路由配置/
...
function loadView(component) {
  return (resolve) =>
    require([
      component ? `@/pages/${component}` : `@/layouts/BlankView`
    ], resolve);
}

// 判断是否为子应用
function isMicroPage(item) {
  if (item.clientCode === 'pc-manage') {
    return item.component;
  } else {
    return `micro-page/${item.clientCode}`;
  }
}

function menuConfig(routesConfig) {
  /**菜单结构*/
  let mentConfig = [];
  /**子菜单*/
  let child_ary = [];
      ...
      m.children.map((r) => {
        ...
        if (r.children && r.children.length > 0) {
          r.children.map((i) => {
            moreChild.push({
              router: i.path,
              name: i.meta ? i.meta.title : null,
              icon: i.meta ? i.meta.icon : null,
              component: loadView(isMicroPage(i))
            });
          });
        }
        child.push({
          router: r.path.split('/')[1],
          name: r.meta ? r.meta.title : null,
          icon: r.meta ? r.meta.icon : null,
          component: loadView(isMicroPage(r)),
          children: [...moreChild]
        });
      });
    }
